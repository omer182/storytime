...
3. `POST /api/stories/:id/generate` `{ length?, generateImages? }` -> requires at least one figure each of category `character`, `location`, `mood`. Missing ones come back as `422 { missing: [...] }`. `length` (`short` | `medium` | `long`, default `medium`) controls the target word count. `generateImages` (default `true`) is a per-story opt-out of illustrations - still requires `GENERATE_IMAGES=true` and `OPENAI_API_KEY` server-side to do anything. On success, calls the configured LLM and saves the story text, then (if enabled both server-side and for this request) generates 3 illustrations - start/middle/end - and returns them as `images: string[]` (base64 data URLs) on this response only. Images are **not persisted** - not written to the db, not saved to disk - so a later `GET` of the same story has no `images` field; they only ever exist in the browser session that just generated them.
